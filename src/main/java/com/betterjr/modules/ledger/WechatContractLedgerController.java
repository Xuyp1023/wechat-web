...
public class WechatContractLedgerController {
    ...
    public @ResponseBody String queryContractLedger(HttpServletRequest request, String queryType, int pageNum, int pageSize) {
        Map<String, Object> map = Servlets.getParametersStartingWith(request, "");
        logger.info("微信端-分页查询贸易合同台账信息，入参:"+ map.toString()+"，queryType="+queryType);
        return exec(() -> contractLedgerService.webQueryContractLedgerByPage(map, queryType, pageNum, pageSize), "微信端-分页查询贸易合同台账", logger);
    }
    ...
    public @ResponseBody String saveContractLedgerFile(Long contractId,String fileTypeName,String fileMediaId) {
        logger.info("微信端-保存合同附件，入参:contractId="+ contractId+"，fileTypeName="+fileTypeName+"，fileMediaId="+fileMediaId);
        return exec(() -> contractLedgerService.webSaveContractLedgerFile(contractId,fileTypeName,fileMediaId), "微信端-添加合同台账", logger);
    }
