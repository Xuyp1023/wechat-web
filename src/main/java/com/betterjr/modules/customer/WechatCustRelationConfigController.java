...
public class WechatCustRelationConfigController {
    ...
    public @ResponseBody String findCustInfo(String custType, String custName) {
        ...
        return exec(() -> custRelationConfigService.webFindCustInfo(custType, findCurrentLongCustNo(), custName),
                "查询客户信息", logger);
    }
    ...
    public @ResponseBody String addCustRelation(String custType, String relationCustStr) {
        ...
        return exec(
                () -> custRelationConfigService.webAddCustRelation(custType, findCurrentLongCustNo(), relationCustStr),
                "添加客户关系", logger);
    }
    ...
    public @ResponseBody String queryCustRelation(String flag, int pageNum, int pageSize, String relationType) {
        return exec(() -> custRelationConfigService.webQueryCustRelation(findCurrentLongCustNo(), flag, pageNum,
                pageSize, relationType), "分页查询客户关系信息", logger);
    }
    ...
    public @ResponseBody String addCustAduitTempFile(Long relateCustNo, String fileTypeName, String fileMediaId,
            String custType) {
        return exec(() -> custRelationConfigService.webAddCustAduitTempFile(relateCustNo, fileTypeName, fileMediaId,
                custType), "保存临时文件", logger);
    }
    ...
    public @ResponseBody String addFactorCustRelation(String factorCustType, String wosCustType,
            String factorCustNoList, String wosCustNoList) {
        logger.info("添加保理方客户关系，入参：custType=" + factorCustType + "，wosCustType=" + wosCustType + "，factorCustNoList="
                + factorCustNoList + "，wosCustStr=" + wosCustNoList);
        return exec(() -> custRelationConfigService.webAddFactorCustRelation(factorCustType, wosCustType,
                factorCustNoList, wosCustNoList, null), "添加保理方客户关系", logger);
    }
